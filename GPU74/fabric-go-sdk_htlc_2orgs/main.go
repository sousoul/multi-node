...
func InitLedger(orgPkSk sdkInit.OrgPkSk, asset []int64, channelIdx int)  {
	...
	txSpeInit := sdkInit.GetRforInit(sdkInit.ZKLedgerCurve, 0, 1, asset[:]) // 当前只有两个组织，Org0是支出方，Org1是接收方
...
func Lock(orgPkSk sdkInit.OrgPkSk, txSpe sdkInit.TxSpecification, orgNum int, hashValue string, T int64, value int64, spenderIdx int, receiverIdx int, channelIdx int, txKey string, flag string) string {
	...
	id1, err := App.Set(arg, channelIdx, 0) // Org1锁定chain1的资产
	if err != nil {
		fmt.Println(err)
	}
	//fmt.Println("<--- org1锁定资产　--->：", id1)
	fmt.Println(fmt.Sprintf("<--- Org%d锁定资产　--->：%s", spenderIdx+1, id1))
...
func Withdraw(preImage, id, txKey string, channel_idx, org_idx int) bool {
	...
	fmt.Println(fmt.Sprintf("<--- Org%d领取资产　--->：%s", org_idx+1, ret))
	if ret=="withdraw successfully"{
		return true // 领取成功
	} else {
		return false
	}
...
func Audit(orgPkSk sdkInit.OrgPkSk, orgNum int, R []*big.Int, spenderIdx int, receiverIdx int, balance, value int64, channelIdx int, txKey string)  {
	...
	ret, err := App.Set(args, channelIdx, 0) // Org1在chain1创建范围证明
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println("<--- 调用audit　--->：", ret)
}
...
func VerifyOne(orgPkSk sdkInit.OrgPkSk, txSpe sdkInit.TxSpecification, orgNum int, channelIdx int, orgIdx int, txKey string)  {
	...
	fmt.Println(fmt.Sprintf("<--- Org%d对账本%d的tx%s进行阶段1验证　--->：", orgIdx+1, channelIdx+1, txKey), ret)
}
		...
		for e := range fbeCh {
			...
			for i, tx := range e.FilteredBlock.FilteredTransactions {
				...
				if eventname=="InvokeEvent_lock" && txVadCode=="VALID"{ // 调用链码中的lock函数发出的事件
					...
					txKey := privateLedger.txKeyList[len(privateLedger.txKeyList)-1] // 获取最新的交易
					if channelIdx==0{
						txSpe := privateLedger.txSpe1List[len(privateLedger.txSpe1List)-1]
						go VerifyOne(orgPkSk, txSpe, orgNum, channelIdx, orgIdx, txKey)
					}else if channelIdx==1{
						txSpe := privateLedger.txSpe2List[len(privateLedger.txSpe2List)-1]
						go VerifyOne(orgPkSk, txSpe, orgNum, channelIdx, orgIdx, txKey)
					}
...
func main() {
	...
	//fmt.Println("channelNum",channelNum, "orgNum", orgNum)
...
func lock(contex *gin.Context) {
	...
	//fmt.Println("资产锁定输入参数：")
	//fmt.Println("channel:", channelIdx)
...
func audit(contex *gin.Context) {
	...
	//Audit(orgPkSk, orgNum, txSpe.R, spenderIdx1, receiverIdx1, asset1[0]-value, value, 0, txKey)
	if channelIdx==0{
		txSpe := privateLedger.txSpe1List[len(privateLedger.txSpe1List)-1]
