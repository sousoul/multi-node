...
// 为每个组织创建msp client和resource management client
func Setup(configFile string, info *SdkEnvInfo) (*fabsdk.FabricSDK, error) {
	// Create SDK setup for the integration tests
	var err error
	sdk, err := fabsdk.New(config.FromFile(configFile)) // 用config.yaml初始化sdk
	if err != nil {
		return nil, err
	}
	...
	for _, org := range info.Orgs {
		org.orgMspClient, err = mspclient.New(sdk.Context(), mspclient.WithOrg(org.OrgName)) // 创建一个msp client
		if err != nil {
			return nil, err
		}
		...
		resMgmtClient, err := resmgmt.New(orgContext) // 创建resource management client
		if err != nil {
...
// 得到的公私钥对应该是SDK客户端的，而crypto-config中保存的是网络中各节点的，所以两者不是一回事。
func GetOrgMsp(info *SdkEnvInfo) error  {
	fmt.Println(">> 获取组织Msp文件......")
	if len(info.Orgs) == 0 {
		return fmt.Errorf("通道组织不能为空，请提供组织信息")
	}

	// 获得所有组织的签名信息
	signIds := []msp.SigningIdentity{} // signIds是一个数组，其中每个元素msp.SigningIdentity{}封装了公私钥对
	for _, org := range info.Orgs {
		// Get signing identity that is used to sign create channel request
		orgSignId, err := org.orgMspClient.GetSigningIdentity(org.OrgAdminUser) // 获得签名信息
		if err != nil {
			return fmt.Errorf("GetSigningIdentity error: %v", err)
		}
		signIds = append(signIds, orgSignId)

		fmt.Println(org.OrgName) // 组织名
		skByte, _ := orgSignId.PrivateKey().Bytes()
		fmt.Println(skByte)  // 以byte数组形式输出该秘钥
		fmt.Println(len(skByte))
		byteToString := string(skByte)
		fmt.Println("byteToString:", byteToString)
	//byteToString: -----BEGIN PRIVATE KEY-----
	//		MHcCAQEEIBjz8It3qkqzgiT2epc6WW4SVuaD7crPHqJqpRU1b0QioAoGCCqGSM49
	//	AwEHoUQDQgAEZn94Nh64cC+KaX8xJMiSVtdP5IV6MSv9s3RkP+zgc2PWspqNuV4t
	//	HPi5GTGDiIBONsf302dHVBlPQi6tGoILaw==
	//	-----END PRIVATE KEY-----
	// 所以私钥文件怎么解析？
		fmt.Println(orgSignId.PrivateKey().Symmetric()) // 是否为对称加密？
		fmt.Println(orgSignId.PrivateKey().Private())  // 该秘钥是否是私钥？
		fmt.Println(orgSignId.PrivateKey().PublicKey()) // 获得该秘钥对应的公钥
	}
	return nil
}
...
func CreateAndJoinChannel(info *SdkEnvInfo) error {
	...
	for _, org := range info.Orgs {
		...
		//if err := org.OrgResMgmt.JoinChannel(info.ChannelID[0], resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint("orderer.example.com")); err != nil {
		//	return fmt.Errorf("%s peers failed to JoinChannel: %v", org.OrgName, err)
		//}
		channelNum := len(info.ChannelID) // 获取通道个数
		for i:=0; i<channelNum; i++ { // 将组织加入所有的通道
			if err := org.OrgResMgmt.JoinChannel(info.ChannelID[i], resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint("orderer.example.com")); err != nil {
				return fmt.Errorf("%s peers failed to JoinChannel: %v", org.OrgName, err)
			}
		}
	}
...
func createChannel(signIDs []msp.SigningIdentity, info *SdkEnvInfo) error {
	...
	//req := resmgmt.SaveChannelRequest{ChannelID: info.ChannelID[0],
	//	ChannelConfigPath: info.ChannelConfig[0],
	//	SigningIdentities: signIDs}
	//
	...
	channelNum := len(info.ChannelID) // 获取通道个数
	for i:=0; i<channelNum; i++ { // 依次创建所有通道
		req := resmgmt.SaveChannelRequest{ChannelID: info.ChannelID[i],
			ChannelConfigPath: info.ChannelConfig[i],
			SigningIdentities: signIDs}
	...
	for i, org := range info.Orgs {
		...
		//req := resmgmt.SaveChannelRequest{ChannelID: info.ChannelID[0],
		//	ChannelConfigPath: org.OrgAnchorFile[0],
		//	SigningIdentities: []msp.SigningIdentity{signIDs[i]}}
		//
		...
		//}
		channelNum := len(info.ChannelID) // 获取通道个数
		for j:=0; j<channelNum; j++ { // org_i获取在channel_j的锚节点文件org.OrgAnchorFile[j]，更新其在这个通道的锚节点配置
			req := resmgmt.SaveChannelRequest{ChannelID: info.ChannelID[j],
				ChannelConfigPath: org.OrgAnchorFile[j],
				SigningIdentities: []msp.SigningIdentity{signIDs[i]}}
...
func CreateCCLifecycle(info *SdkEnvInfo, sequence int64, upgrade bool, sdk *fabsdk.FabricSDK) error {
	...
	//if err := approveCC(packageID, info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID[0], info.Orgs, info.OrdererEndpoint); err != nil {
	//	return fmt.Errorf("approveCC error: %v", err)
	//}
	//
	//// Query approve cc
	////if err:=queryApprovedCC(info.ChaincodeID, sequence, info.ChannelID, info.Orgs);err!=nil{
	//if err:=queryApprovedCC(info.ChaincodeID, sequence, info.ChannelID[0], info.Orgs);err!=nil{
	...
	//if err:=checkCCCommitReadiness(packageID, info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID[0], info.Orgs); err!=nil{
	...
	//if err:=commitCC(info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID[0], info.Orgs, info.OrdererEndpoint);err!=nil{
	//	return fmt.Errorf("commitCC error: %v", err)
	//}
	//// Query committed cc
	////if err:=queryCommittedCC(info.ChaincodeID, info.ChannelID, sequence, info.Orgs); err!=nil{
	//if err:=queryCommittedCC(info.ChaincodeID, info.ChannelID[0], sequence, info.Orgs); err!=nil{
	...
	//if err:=initCC(info.ChaincodeID, upgrade, info.ChannelID[0], info.Orgs[0], sdk); err!=nil{
	...
	for i:=0; i<channelNum; i++ {
		fmt.Println(">>>>>>>通道", info.ChannelID[i])

		// Approve cc
		fmt.Println(">> 组织认可智能合约定义......")
		//if err := approveCC(packageID, info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID, info.Orgs, info.OrdererEndpoint); err != nil {
		if err := approveCC(packageID, info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID[i], info.Orgs, info.OrdererEndpoint); err != nil {
			return fmt.Errorf("approveCC error: %v", err)
		}

		// Query approve cc
		//if err:=queryApprovedCC(info.ChaincodeID, sequence, info.ChannelID, info.Orgs);err!=nil{
		if err:=queryApprovedCC(info.ChaincodeID, sequence, info.ChannelID[i], info.Orgs);err!=nil{
			return fmt.Errorf("queryApprovedCC error: %v", err)
		}
		fmt.Println(">> 组织认可智能合约定义完成")
		...
		fmt.Println(">> 检查智能合约是否就绪......")
		//if err:=checkCCCommitReadiness(packageID, info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID, info.Orgs); err!=nil{
		if err:=checkCCCommitReadiness(packageID, info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID[i], info.Orgs); err!=nil{
			return fmt.Errorf("checkCCCommitReadiness error: %v", err)
		}
		fmt.Println(">> 智能合约已经就绪")
		...
		fmt.Println(">> 提交智能合约定义......")
		//if err:=commitCC(info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID, info.Orgs, info.OrdererEndpoint);err!=nil{
		if err:=commitCC(info.ChaincodeID, info.ChaincodeVersion, sequence, info.ChannelID[i], info.Orgs, info.OrdererEndpoint);err!=nil{
			return fmt.Errorf("commitCC error: %v", err)
		}
		// Query committed cc
		//if err:=queryCommittedCC(info.ChaincodeID, info.ChannelID, sequence, info.Orgs); err!=nil{
		if err:=queryCommittedCC(info.ChaincodeID, info.ChannelID[i], sequence, info.Orgs); err!=nil{
			return fmt.Errorf("queryCommittedCC error: %v", err)
		}
		fmt.Println(">> 智能合约定义提交完成")
		...
		fmt.Println(">> 调用智能合约初始化方法......")
		//if err:=initCC(info.ChaincodeID, upgrade, info.ChannelID, info.Orgs[0], sdk); err!=nil{
		if err:=initCC(info.ChaincodeID, upgrade, info.ChannelID[i], info.Orgs[0], sdk); err!=nil{
			return fmt.Errorf("initCC error: %v", err)
		}
		fmt.Println(">> 完成智能合约初始化")
...
func approveCC(packageID string, ccName, ccVersion string, sequence int64, channelID string, orgs []*OrgInfo, ordererEndpoint string) error {
	...
	//ccPolicy := policydsl.SignedByNOutOfGivenRole(int32(len(mspIDs)), mb.MSPRole_MEMBER, mspIDs) // 所有组织都背书
	ccPolicy := policydsl.SignedByNOutOfGivenRole(1, mb.MSPRole_MEMBER, mspIDs) // 只有一个组织背书
...
func checkCCCommitReadiness(packageID string, ccName, ccVersion string, sequence int64, channelID string, orgs []*OrgInfo) error {
	...
	ccPolicy := policydsl.SignedByNOutOfGivenRole(1, mb.MSPRole_MEMBER, mspIds)
...
// http://www.iawfy.net/index.php/archives/30/ initcc和该链接中完全一样