...
type ChaincodeStubInterface interface {
	// API的首字母需要大写，否则无法调用
	// 密码学原语API
	Commitment(value *big.Int, r *big.Int) zksigma.ECPoint
	Token(pk zksigma.ECPoint, r *big.Int) zksigma.ECPoint
	Test_bulletproof() string
	Create_bulletproof(m *big.Int) (RangeProof, *big.Int)
	Verify_bulletproof(rtn RangeProof) bool
	// My API for test
	Cal_commitment(amount string) string

	/*FabZK chaincode API*/
	// Compute commitments and audit tokens
	ZkPutState()
	ZkPutState_test(txSpeJsons string, key string) (string, error)
	//ZkPutState_test() (string, error)
	// Compute range proofs and disjunctive proofs
	ZkAudit(auditSpeJsons string, key string) (string, error)
	//Verify the proofs against the input from client code
	ZkVerifyOne(key string, orgNum int, sk *big.Int, org_name string, value *big.Int) (string, error)
	ZkVerifyTwo(key string, orgNum int, org_name string, sk *big.Int) (string, error)
	ZkVerifyTwoAll(key string, orgNum int) (string, error)
